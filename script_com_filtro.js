...
function renderConsolidatedReport(container) {
    ...
    allStatesData.forEach(state => {
        Object.entries(state.monthly).forEach(([period, d]) => {
            ...
            allPeriods[period].pagoAte12 += d.pagoAte12;
    ...
    Object.values(allPeriods).forEach(d => {
        ...
        globalTotals.pagoAte12 += d.pagoAte12;
    ...
    filteredPeriods.forEach(p => {
        ...
        filteredTotals.pagoAte12 += d.pagoAte12;
    ...
    reportDiv.innerHTML = `
        <div class="state-title">
            <span>🌍</span> CONSOLIDADO GERAL (PLANILHA ATUAL + HISTÓRICO)
        </div>
        ...
        <div class="table-container">
            <table>
                <thead>
                    <tr>
                        <th>Período</th>
                        <th class="text-right">Emprest. (Até 12)</th>
                        <th class="text-right">Previsto (Até 12)</th>
                        <th class="text-right">Pago (Até 12)</th>
                ...
                <tbody>
                    ${filteredPeriods.map(p => {
                        ...
                        return `
                            <tr>
                                ...
                                <td class="text-right">${formatCurrency(d.emprestadoAte12)}</td>
                                <td class="text-right">${formatCurrency(d.previstoAte12)}</td>
                                <td class="text-right">${formatCurrency(d.pagoAte12)}</td>
                    ...
                    <tr class="totals-row">
                        ...
                        <td class="text-right">${formatCurrency(filteredTotals.emprestadoAte12)}</td>
                        <td class="text-right">${formatCurrency(filteredTotals.previstoAte12)}</td>
                        <td class="text-right">${formatCurrency(filteredTotals.pagoAte12)}</td>
        ...
        <div class="table-container">
            <table>
                ...
                <tbody>
                    ...
                    <tr class="totals-row">
                        <td>TOTAL FILTRADO</td>
                        <td class="text-right">${formatCurrency(totals.emprestadoAte12)}</td>
                        <td class="text-right">${formatCurrency(totals.previstoAte12)}</td>
                        <td class="text-right">${formatCurrency(totals.pagoAte12)}</td>
